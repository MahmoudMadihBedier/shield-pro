...
export class SalesService implements ISalesService {
  ...
  async createCustomer(customer: Omit<Customer, 'id' | 'created_at' | 'updated_at'>): Promise<Customer & { client_id?: string; crm_credentials?: { tempPassword: string } }> {
    ...
    // Automatically create CRM account if email is provided
    let crmCredentials = undefined;
    if (customer.email) {
      try {
        const crmResult = await CRMService.createClientUser({
          name: customer.name,
        ...
        if (crmResult.success && crmResult.tempPassword && crmResult.user_id) {
          // Link the CRM user to the existing customer
          const linkResult = await CRMService.linkCrmUserToCustomer(newCustomer.id, crmResult.user_id);
          if (linkResult.success) {
            crmCredentials = { tempPassword: crmResult.tempPassword };
          } else {
            console.warn('Could not link CRM user to customer, but auth user was created');
            // Still return credentials since the user can login
            crmCredentials = { tempPassword: crmResult.tempPassword };
          }
        }
      } catch (error) {
        console.error('Error creating CRM account:', error);
        // Don't fail customer creation if CRM account creation fails
      }
    }
    
    // Return the customer with the generated client_id and CRM credentials
    return { 
      ...newCustomer, 
      client_id: (newCustomer as any).client_id,
      crm_credentials: crmCredentials
    } as Customer & { client_id?: string; crm_credentials?: { tempPassword: string } };
  }
