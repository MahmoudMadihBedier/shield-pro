...
import { queueOfflineWrite, triggerSync, pullFromServer, subscribeToSync } from '../../infrastructure/sync/sync-service';
import { getSetting, getSettingBool, saveSetting } from '../../shared/utils/settings-helper';
import {
  Save,
  Plus,
  Trash2,
...
export const Settings: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToSync((state: any) => {
      setSyncState(state);
    });
    return () => unsub();
  ...
  const loadGeneralSettings = async () => {
    ...
    setGpsTrackedRoleIds((await getSetting('gps_tracking_role_ids', '')).split(',').map((s: string) => s.trim()).filter(Boolean));
  };
